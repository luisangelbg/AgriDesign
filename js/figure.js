...
/* ================= palettes ================= */
Fig.palettes = {
  agri:     ['#2f7d4f', '#c8842a', '#2b7bb9', '#b5432f', '#7a5195', '#5aa469', '#e0b04c', '#3b9db3', '#8c5a3c', '#6d6e71'],
...
Fig.paletteNames = {
  agri: 'AgriDesign (field)', harvest: 'Harvest', soil: 'Soil & crop', npg: 'Nature (NPG)', lancet: 'Lancet',
...
Fig.color = (palette, i) => {
  const p = Fig.palettes[palette] || Fig.palettes.agri;
  return p[i % p.length];
};
  ...
  groups.forEach((gr, gi) => {
    ...
    gr.controls.forEach(c => {
      ...
      if (c.type === 'select') {
        ...
        input = mk('input', { type: 'color', value: cfg[c.key] || '#2f7d4f' });
